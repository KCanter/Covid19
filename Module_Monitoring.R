...
MonModuleUI <- function(id, data) {
    ...
    shiny::fluidRow(
        ...
        shinydashboard::tabBox(
            ...
            id = ns("tabset"), height = "500px",
            width = 10,
        tabPanel("Casos confirmados", 
                 shinydashboard::box(solidHeader = T, width = 6,
        ...
        shinydashboard::box(
            ...
            dateRangeInput(ns("fechas"), label = "Rango de fechas", 
                           start = "2020-03-25", end = "2021-06-30", 
                           startview = "month")
        )
    )
}
...
MonModulerSR <- function(input, output, session, data) {
    ...
    fil_data <- reactive({
            ...
            dplyr::filter(ciudad_municipio_nom == input$selectMun, 
                          Fecha >= input$fechas[1] & Fecha <= input$fechas[2]) 
        })
    ...
    output$Casos <- renderDygraph({
        ...
        data_xts <- xts::xts(x = Casos_fil[, 2:4], order.by = Casos_fil$Fecha)
    ...
    output$Recuperados <- renderDygraph({
        ...
        Casos_fil <- 
            ...
            dplyr::count(Fecha, sexo) %>% 
            tidyr::spread(sexo, n) %>% 
            dplyr::rename("MRecuperadas" = "F", "HRecuperados" = "M") %>% 
            dplyr::rowwise(Fecha) %>% 
    ...
    output$recuperados_edad_sexo <- renderPlot({
        fil_data() %>% 
            dplyr::filter(recuperado == "Recuperado") %>% 
            dplyr::mutate(edad = as.numeric(edad), sexo = as.factor(sexo), 
                   fuente_tipo_contagio = as.factor(fuente_tipo_contagio), 
                   edad_group = dplyr::case_when(
                       edad < 1            ~ "Menor a 1 año",
                       edad >= 1 & edad <= 9 ~ "1 a 9 años",
